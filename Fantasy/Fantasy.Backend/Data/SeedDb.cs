...
public class SeedDb
{
    ...
    public async Task SeedAsync()
    {
        ...
        await CheckUserAsync("Fernando", "Franco", "ffranco@adinet.com.uy", "+56 9 84787772", UserType.Admin);
    ...
    private async Task<User> CheckUserAsync(string firstName, string lastName, string email, string phone, UserType userType)
    {
        ...
        if (user == null)
        {
            var country = await _context.Countries.FirstOrDefaultAsync(x => x.Name == "Uruguay");
            user = new User
            {
                FirstName = firstName,
                LastName = lastName,
